...
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- heuristique conservée pour reco automatique
function topRecommendations(result: FrictionResult, allPacks: AutomationPack[]) {
  // Heuristique simple : plus le score est élevé, plus on met en avant
  // les packs à fort impact structurel (synchronisation, événements).
  void result
  const priorityOrder = ['sync', 'evenements', 'reporting', 'documents', 'onboarding', 'affectation']
  return priorityOrder
    .map((id) => allPacks.find((p) => p.id === id))
    .filter((p): p is AutomationPack => Boolean(p))
    .slice(0, 3)
}

export function ResultsAndCapture({ result, selectedPacks }: Props) {
  const [isPending, startTransition] = useTransition()
  const [status, setStatus] = useState<'idle' | 'sent' | 'error'>('idle')
  const [form, setForm] = useState({ nom: '', cabinet: '', email: '' })

  const allPacksForReco = selectedPacks.length > 0 ? selectedPacks : []
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await submitDiagnostic({
        ...
        packsSelectionnes: selectedPacks.map((p) => p.titre),
      })
